...
def workloadScoringByStatusesChannels(Data,NumOfAllDays,NumOfIntervalDays):
    ...
    #splitting by status
    statuses = DataFrame.status.unique()
    channels = DataFrame.channel.unique()
    assignee_id_list = []
    status_list = []
    channel_list = []
    avg_num_of_task_per_week_list = []
    ...
    for status in statuses:
        for channel in channels:
            dataframe_status_channel = Data[(Data.status == str(status)) & (Data.channel == str(channel))][:]
            ...
            num_tasks_per_week = []
            for i in range(0,num_of_intervals):
                interval = dataframe_status_channel[(dataframe_status_channel.updated >= str(first_date)) & (dataframe_status_channel.updated <= str(first_interval))][:]
                first_date = first_date + delta_interval
                first_interval = first_interval + delta_interval
...
def workloadTotalScore(InsertDataFrame):
    scores = pd.DataFrame([{"assignee_id": np.nan,"score_total":np.nan}], dtype='int32').dropna()
    for i in InsertDataFrame.assignee_id.unique():
        scores = scores.append({"assignee_id": i,"score_total":int(InsertDataFrame[InsertDataFrame.assignee_id == i].score_value.max())}, ignore_index=True)
    scores['assignee_id'] = scores['assignee_id'].astype('int64')
    return scores
...
def workloadAllAssigneesTotal(InsertDataFrame,timerange,timedel):
    scores_total = pd.DataFrame()
    for i in InsertDataFrame.assignee_id.unique():
        test_user = InsertDataFrame[InsertDataFrame.assignee_id == i][:]
        test_user.reset_index(inplace=True, drop=True)
        test_result = workloadScoringByStatuses(test_user,timerange,timedel)
        test_total = workloadTotalScore(test_result)
        scores_total = scores_total.append(test_total, ignore_index=True)
    scores_total['assignee_id'] = scores_total['assignee_id'].astype('int64')
    return scores_total
        
def workloadAllAssignees(InsertDataFrame,timerange,timedel):
    scores = pd.DataFrame()
    for i in InsertDataFrame.assignee_id.unique():
        test_user = InsertDataFrame[InsertDataFrame.assignee_id == i][:]
        test_user.reset_index(inplace=True, drop=True)
        test_result = workloadScoringByStatuses(test_user,timerange,timedel)
        scores  = scores.append(test_result)
    test_result['assignee_id'] = test_result['assignee_id'].astype('int64')                                        
    return scores  

def workloadAllAssigneesStatusesChannels(InsertDataFrame,timerange,timedel):
    scores = pd.DataFrame()
    for i in InsertDataFrame.assignee_id.unique():
        test_user = InsertDataFrame[InsertDataFrame.assignee_id == i][:]
        test_user.reset_index(inplace=True, drop=True)
        test_result = workloadScoringByStatusesChannels(test_user,timerange,timedel)
        scores  = scores.append(test_result)
    test_result['assignee_id'] = test_result['assignee_id'].astype('int64')                                        
    return scores 

def workloadAllAssigneesStatusesChannelsTotal(InsertDataFrame,timerange,timedel):
    scores_total = pd.DataFrame()
    for i in InsertDataFrame.assignee_id.unique():
        test_user = InsertDataFrame[InsertDataFrame.assignee_id == i][:]
        test_user.reset_index(inplace=True, drop=True)
...
def insertScoreResultData(InsertDataFrame,ProjectId,DatasetId,TableId,IfExists):
    ...
    res_df['developer'] = 'kirill.zubarev'
    res_df['developer'] = res_df['developer'].astype('str')

    try:
        pandas_gbq.to_gbq(res_df, destination_table=destination_table, project_id=ProjectId, if_exists=IfExists) # if_exists='append','replace','fail
    except:
        print('insertScoreResultDataTotalError: destination table exists')

def insertScoreResultDataStatusesChannels(InsertDataFrame,ProjectId,DatasetId,TableId,IfExists):
    destination_table = f"{DatasetId}.{TableId}"
...
def insertScoreResultDataTotal(InsertDataFrame,ProjectId,DatasetId,TableId,IfExists):
    destination_table = f"{DatasetId}.{TableId}"
    
    res_df = pd.DataFrame()
    res_df['assignee_id'] = InsertDataFrame['assignee_id'].astype('int64')
    res_df['score_value'] = InsertDataFrame['score_total'].astype('float32')
    res_df['developer'] = 'kirill.zubarev'
    res_df['developer'] = res_df['developer'].astype('str')
